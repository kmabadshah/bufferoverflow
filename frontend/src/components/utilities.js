...
export function error_log(e, show_random_error) {
  ...
  if (show_random_error) {
    store.dispatch(extras_actions.random_error_on())
  }
}
...
export const ws_connect = () => {
  ...
  ws.addEventListener('error', (e) => console.log(`ERROR HAPPENED`))
  ...
  ws.addEventListener('message', async e => {
    ...
    if (event === `created`) {
      if (data[`table`] === `questions`) {
        ...
        if (res.status !== 200)
          throw res
        store.dispatch(answer_comments_actions.add(res.data))
      }
